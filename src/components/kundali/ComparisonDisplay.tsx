...
export const ComparisonDisplay: React.FC<ComparisonDisplayProps> = ({ result, onReturnToForm }) => {
  ...
  return (
    <div className="kundali-display-container flex flex-col  bg-slate-200 rounded-xl shadow-lg w-full lg:max-w-4xl xl:max-w-6xl lg:p-4 md:p-1 space-y-4 text-sm md:text-base dark:bg-gray-800 dark:text-gray-200 dark:border dark:border-gray-700">
      ...
      {/* Groom and Bride Details Section */}
      ...
      <div className="kundali-card p-4 sm:p-6 rounded-lg overflow-x-auto bg-slate-200 dark:bg-gray-800">
        ...
        <table className="w-full text-left text-sm sm:text-base">
          ...
          <tfoot className="border-t-2 border-amber-300 dark:border-amber-600 font-bold">
            <tr>
              <td colSpan={3} className="py-2 px-2 text-right text-stone-800 dark:text-stone-100">{NEPALI_LABELS.totalPoints}</td>
              <td className="py-2 px-2 text-center text-stone-800 dark:text-stone-100">{toDevanagari(36)}</td>
              <td className="py-2 px-2 text-center text-lg text-blue-600 dark:text-blue-400">{toDevanagari(score.total)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      <div className="kundali-card rounded-lg bg-slate-200 dark:bg-gray-800">
        <h3 className="text-xl font-bold text-blue-400 dark:text-blue-400">
          {NEPALI_LABELS.conclusion}
        </h3>
      ...
      {typeof window !== 'undefined' && (
        <div className="print:hidden">
          ...
          {/* Spacer for screen view only; collapsed in print to avoid blank page */}
          <div className="h-32 print:h-0 print:overflow-hidden" aria-hidden="true"></div>

          {/* Fixed footer with buttons, hidden in print */}
          <div className="fixed bottom-12 left-1/2 transform -translate-x-1/2 z-[999] w-full px-4 text-center print:hidden">
